...
<script type="text/javascript" src="resources/js/guestbook/guestbook.js?after"></script>
...
<body>
   ...
   <input type="hidden" name="sendTo" value="${dto.username }" />
   <div class="culmn">
      <div class="column "
            ...
            <div
               ...
               <c:choose>
                  <c:when test="${empty guestlist }">
                     <h3 class="text-center" id="NO">방명록이 존재하지 않습니다.</h3> 
                  </c:when>
                  <c:otherwise>
                     <c:forEach items="${guestlist }" var="babo" varStatus="status">
                        <div>
                           <div id="GuestHide${babo.boardnum }">
                              ...
                              <table>
                                 <ul class="breadcrumb">
                                    ...
                                    <c:if test="${babo.secret == 'Y' }">
                                    
                                    <li>(${babo.reporting_date}) <p class="glyphicon glyphicon-lock" id="secret_post">비밀글</p></li> 
                                    </c:if>  
                                    <c:if test="${babo.secret == 'N' }">  
                              ...
                              <div style="border: gray;">
                                 <div>
                                    <div class="big"
                                       ...
                                       <div class="small">${guestdto[status.index].usermini}</div>
                                    </div>
                                    <div style="margin-right: 5%; margin-left: 20%;">
                                       <textarea rows="12" cols="50" class="text form-control"
                                 ...
                                 <br />
                                 <div
                                    style="overflow: hidden; background-color: #fffcd2; border-radius: 4px;">
                                    <table id="commTable${babo.boardnum }">
                                       <c:choose>
                                          ...
                                          <c:otherwise>
                                             <c:forEach items="${comm }" var="comment">
                                                <c:if test="${comment.boardnum == babo.boardnum}">
                                                   <tr id="commnum${comment.commnum}">
                                                      <td id="commid${comment.commnum}"><a href=""
                                                         style="font-weight: bold;">${comment.username}</a> <br></td>
                                                      <td id="comment${comment.commnum}">${comment.guestbookcomm}</td>
                                                      <td style="font-size: 12px;">(${comment.reporting_date})</td>
                                                      <c:if test="${username eq comment.username }">
                                                         <td><button type="button"
                                                               class="btn btn-default btn-sm" id="comment_btn"
                                                               onclick="guestBookCommDelete(${comment.commnum})">
      ...
      <nav class="nav ">
         <ul class="nav flex-column">
            ...
            <li class="nav-item "
               ...
               class="nav-link" href="visitor.do" style="color: black;">방명록</a></li>
