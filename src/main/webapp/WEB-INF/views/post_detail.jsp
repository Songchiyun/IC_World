...
<!-- Demo styles -->
<link rel="stylesheet" type="text/css"
	href="resources/css/post_detail.css?after">
               ...
               $(".comment_box"+commnum).append( 
                  "<div id='hide_comm_box"+commnum+"' class='hide_comm_box hide_comm_box$"+commnum+" hide_comm_box_gn"+groupno+"'>"+   
                     "<div class='comment'><ul><li>"+
                        "<a class='profile_img'> ${commdto[status.index].usermini}</a>"+
                        "<a class='comment_a' href='friendhome.do?username="+msg.username+"'>"+ msg.username +"</a><span class='postcomm_span'>"+ msg.content +"</span></li></ul>"+
                        "<!-- <button class='more_btn'><span class='more' aria-label='옵션 더 보기'></span></button> -->"+
                     "</div>"+
                     "<input type='button' class='del_btn' onclick='delete_add_comm("+commnum+")' value='삭제'>"
	...
	<div class="modal-body">
		<div class="right_container">
				<div class="right_header">
					...
					<div class="header_id">
					<c:choose>
						<c:when test = "${myname == dto.username }">
						<a class="comment_a"
							href="home.do">${dto.username}</a>
						</c:when>
						<c:otherwise>
						<a class="comment_a"
							href="friendhome.do?username=${dto.username }">${dto.username}</a>
						</c:otherwise>
					</c:choose>
					</div>
				...
				<div id="right_body" style="-ms-overflow-style: none;">
					<div class="postcomm">
						<a class="profile_img">${myusermini.usermini}</a> <a
							class="comment_a" href="friendhome.do?username=${dto.username }">
							${dto.username}</a> <span
							style="margin-left: 0.5em;">${dto.postcomm }</span>
								...
								<c:forEach items="${comm }" var="comment" varStatus="status">
									<c:if
										...
										<div class="comment_box${comment.commnum}">
											<div class="comment">
												<ul>
													<li><a class="profile_img"> <!-- 여기에 미니미이미지 들어가야함!!! -->
															${commdto[status.index].usermini}
													</a> <a class="comment_a${comment.commnum} comment_a"
														id="comment_a"
														href="friendhome.do?username=${comment.username }">${comment.username}
											...
											<div class="comment">
												<ul>
													<li><a class="profile_img">
															${commdto[status.index].usermini}</a> <a class="comment_a"
														id="comment_a${comment.commnum}">${comment.username} </a><span
